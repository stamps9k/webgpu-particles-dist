// src/internal/WebGPUContext.ts
var WebGPUContext = class _WebGPUContext {
  adapter;
  device;
  context;
  format;
  // -- The buffers used in simulation
  uniform_buffers;
  // -- Timing
  last_time = performance.now();
  /**
   *
   * Construct a new particle engine instance.
   * Called by the static init function to enable the async nature of the
   * generation of WebGPU contexts
   *
   * @param adapter - The standard WebGPU Adapter
   * @param device - The standard WebGPU device
   * @param context - The standard WebGPU context
   * @param format  - The standard WebGPU Texture format
   * @param shaders - A list of all shaders used by WebGPU
   * @param buffers - A list of all buffers used by WebGPU
   * @param compute_pipeline - The standard WebGPU Compute Pipeline
   * @param render_pipeline - The standard WebGPU Render Pipeline
   * @param bind_groups - A list of all bindgroups used by WebGPU
   */
  constructor(adapter, device, context, format, uniform_buffers) {
    this.adapter = adapter;
    this.device = device;
    this.context = context;
    this.format = format;
    this.uniform_buffers = uniform_buffers;
  }
  // -------------------------------------------------------------------------
  // Initialisation
  // -------------------------------------------------------------------------
  /**
   *
   * asyncronous init code
   *
   * @param canvas - The canvas element in the browser DOM
   * @param options - Any WebGPU options provided by the user
   * @param shaders_text - The text of each shader used by the program
   * @param max_particles - The max number of particles requested by the user
   * @param particle_stride - The stride of each particle
   * @returns
   */
  static async init(canvas, options = {}) {
    const particle_stride = 48;
    if (!navigator.gpu) {
      throw new Error("WebGPU is not supported in this environment.");
    }
    const adapter = await navigator.gpu.requestAdapter({
      powerPreference: options.powerPreference ?? "high-performance"
    });
    if (!adapter) {
      throw new Error("No GPUAdapter found. WebGPU may not be available.");
    }
    const device = await adapter.requestDevice({
      label: "webgpu-main-device",
      requiredFeatures: [],
      requiredLimits: {}
    });
    device.lost.then((info) => {
      console.error(
        `GPU device lost: ${info.message} (reason: ${info.reason})`
      );
    });
    const context = canvas.getContext("webgpu");
    if (!context) {
      throw new Error("Failed to get WebGPU canvas context.");
    }
    const format = navigator.gpu.getPreferredCanvasFormat();
    context.configure({
      device,
      format,
      alphaMode: "premultiplied"
    });
    let uniform_buffers = {};
    uniform_buffers["uniform_buffer"] = device.createBuffer({
      label: "uniforms",
      size: 80,
      usage: GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST
    });
    return new _WebGPUContext(adapter, device, context, format, uniform_buffers);
  }
  // -------------------------------------------------------------------------
  // Frame helpers
  // -------------------------------------------------------------------------
  /**
   *
   * Return the current swap-chain texture view to use as the render target.
   *
   * @returns the current swap-chain texture view
   *
   */
  get_current_color_view() {
    return this.context.getCurrentTexture().createView();
  }
  /**
   * Create a basic render pass descriptor pointed at the current
   * swap-chain texture. Extend or replace as needed.
   *
   * @param clearColor - the clear color
   * @returns the GPU Render Pass
   *
   */
  create_render_pass_descriptor(clearColor = { r: 0, g: 0, b: 0, a: 1 }) {
    return {
      colorAttachments: [
        {
          view: this.get_current_color_view(),
          clearValue: clearColor,
          loadOp: "clear",
          storeOp: "store"
        }
      ]
    };
  }
  /**
   * Create a command encoder to encode a list of commands to be consumed by the GPU
   *
   * @returns a command encoder
   */
  begin_frame() {
    return this.device.createCommandEncoder({ label: "frame-encoder" });
  }
  /**
   * Finish the encoder and pass to the device for processing
   *
   * @param encoder - the encoder to close
   */
  end_frame(encoder) {
    this.device.queue.submit([encoder.finish()]);
  }
  // -------------------------------------------------------------------------
  // Resource helpers
  // -------------------------------------------------------------------------
  /**
   *
   * Write some data the GPU's buffer
   *
   * @param buffer_name - The buffer to write to
   * @param bufferOffset - How much to offset the write command
   * @param data - The data to write to the buffer
   */
  write_buffer(canvas, uniform_values) {
    const uniform_data = new Float32Array([
      uniform_values.delta_time,
      uniform_values.time,
      canvas.width,
      canvas.height,
      uniform_values.emitter_pos[0],
      uniform_values.emitter_pos[1],
      uniform_values.emitter_shape,
      uniform_values.emitter_p1,
      uniform_values.emitter_p2,
      uniform_values.sp0,
      uniform_values.sp1,
      uniform_values.sp2,
      uniform_values.sp3,
      uniform_values.sp4,
      uniform_values.sp5,
      uniform_values.sp6,
      uniform_values.sp7,
      uniform_values.sp8,
      uniform_values.sp9,
      uniform_values.sp10
    ]);
    this.device.queue.writeBuffer(
      this.uniform_buffers["uniform_buffer"],
      0,
      uniform_data
    );
  }
  get_uniform_buffers = () => {
    return this.uniform_buffers;
  };
  // -------------------------------------------------------------------------
  // Cleanup
  // -------------------------------------------------------------------------
  /** Clean up the WebGPU context when finished */
  destroy() {
    this.context.unconfigure();
    this.device.destroy();
  }
};

// src/internal/shaders/scatter-fade/scatter-fade.frag.wgsl
var scatter_fade_frag_default = "struct FragmentIn { @location(0) color : vec4f, @location(1) uv : vec2f, } @fragment fn fs_main(in: FragmentIn) -> @location(0) vec4f { let centered = in.uv * 2.0 - 1.0; let dist = length(centered); if (dist > 1.0) { discard; } let alpha = smoothstep(1.0, 0.0, dist) * in.color.a; return vec4f(in.color.rgb, alpha); }";

// src/internal/shaders/scatter-fade/scatter-fade.vert.wgsl
var scatter_fade_vert_default = "struct Uniforms { deltaTime : f32, time : f32, canvasSize: vec2f, emitterPos : vec2f, emitterType: f32, emitterP1 : f32, emitterP2 : f32, sp0 : f32, sp1 : f32, sp2 : f32, sp3 : f32, sp4 : f32 } struct Particle { position : vec2f, velocity : vec2f, color : vec4f, seed : f32, life : f32, maxLife : f32, size : f32 } @group(0) @binding(0) var<uniform> uniforms : Uniforms; @group(0) @binding(1) var<storage, read> particles : array<Particle>; struct VertexOut { @builtin(position) clipPosition : vec4f, @location(0) color : vec4f, @location(1) uv : vec2f, } const QUAD_POS = array<vec2f, 6>( vec2f(-1.0, -1.0), vec2f( 1.0, -1.0), vec2f( 1.0, 1.0), vec2f(-1.0, -1.0), vec2f( 1.0, 1.0), vec2f(-1.0, 1.0), ); const QUAD_UV = array<vec2f, 6>( vec2f(0.0, 1.0), vec2f(1.0, 1.0), vec2f(1.0, 0.0), vec2f(0.0, 1.0), vec2f(1.0, 0.0), vec2f(0.0, 0.0), ); @vertex fn vs_main( @builtin(vertex_index) vIdx : u32, @builtin(instance_index) pIdx : u32, ) -> VertexOut { let p = particles[pIdx]; let corner = QUAD_POS[vIdx]; let uv = QUAD_UV[vIdx]; let pixelPos = p.position + corner * p.size; let clipPos = (pixelPos / uniforms.canvasSize) * 2.0 - 1.0; var out: VertexOut; out.clipPosition = vec4f(clipPos.x, -clipPos.y, 0.0, 1.0); out.color = p.color; out.uv = uv; return out; }";

// src/internal/shaders/scatter-fade/scatter-fade.comp.wgsl
var scatter_fade_comp_default = "struct Uniforms { deltaTime : f32, time : f32, canvasSize: vec2f, emitterPos : vec2f, emitterType: f32, emitterP1 : f32, emitterP2 : f32, sp0 : f32, sp1 : f32, sp2 : f32, sp3 : f32, sp4 : f32 } struct Particle { position : vec2f, velocity : vec2f, color : vec4f, seed : u32, life : f32, maxLife : f32, size : f32, } @group(0) @binding(0) var<uniform> uniforms : Uniforms; @group(0) @binding(1) var<storage, read_write> particles : array<Particle>; fn pcg(v: u32) -> u32 { let state = v * 747796405u + 2891336453u; let word = ((state >> ((state >> 28u) + 4u)) ^ state) * 277803737u; return (word >> 22u) ^ word; } fn rand_f32(seed: u32) -> f32 { return f32(pcg(seed)) / f32(0xffffffffu); } fn respawn(i: u32) -> Particle { var p: Particle; let r1 = rand_f32(pcg(i)); let r2 = rand_f32(pcg(i + 100000u)); let r3 = rand_f32(pcg(i + 200000u)); let r4 = rand_f32(pcg(i + 300000u)); let r5 = rand_f32(pcg(i + 400000u)); let r6 = rand_f32(pcg(i + 500000u)); let r7 = rand_f32(pcg(i + 600000u)); var spawnPos: vec2f; if (uniforms.emitterType == 0.0) { spawnPos = uniforms.emitterPos; } else if (uniforms.emitterType == 1.0) { let angle = rand_f32(i * 7) * 6.2832; let radius = sqrt(rand_f32(i * 8)) * uniforms.emitterP1; spawnPos = uniforms.emitterPos + vec2f(cos(angle), sin(angle)) * radius; } else if (uniforms.emitterType == 2.0) { let rx = (rand_f32(i * 7) - 0.5) * uniforms.emitterP1; let ry = (rand_f32(i * 8) - 0.5) * uniforms.emitterP2; spawnPos = uniforms.emitterPos + vec2f(rx, ry); } p.position = spawnPos; p.velocity = vec2f((r1 - 0.5) * 400.0, -(r2 * 300.0 + 100.0)); p.color = vec4f(r3, r4, 1.0 - r3, 1.0); p.seed = pcg(i + u32(uniforms.time * 1000.0)); p.life = 1.0; p.maxLife = r7 * 2.0 + 0.5; p.size = r1 * 8.0 + 4.0; return p; } @compute @workgroup_size(64) fn main(@builtin(global_invocation_id) id: vec3u) { let i = id.x; if (i >= arrayLength(&particles)) { return; } var p = particles[i]; p.life -= uniforms.deltaTime / p.maxLife; if (p.life <= 0.0) { particles[i] = respawn(i); return; } p.velocity.y += 100 * uniforms.deltaTime; p.position += p.velocity * uniforms.deltaTime; p.color.a = p.life; particles[i] = p; }";

// src/internal/shaders/scatter-swirl/scatter-swirl.frag.wgsl
var scatter_swirl_frag_default = "struct VertexOut { @builtin(position) pos : vec4f, @location(0) uv : vec2f, @location(1) color : vec4f, } @fragment fn fs_main(in: VertexOut) -> @location(0) vec4f { let d = length(in.uv); if (d > 1.0) { discard; } let core = 1.0 - smoothstep(0.0, 0.6, d); let glow = pow(core, 2.0); let life = in.color.a; let hotColor = vec3f(0.85, 1.00, 1.00); let coldColor = vec3f(0.15, 0.00, 0.40); let baseColor = mix(coldColor, hotColor, pow(life, 0.5)); return vec4f(baseColor * glow, glow * life); }";

// src/internal/shaders/scatter-swirl/scatter-swirl.vert.wgsl
var scatter_swirl_vert_default = "struct Uniforms { deltaTime : f32, time : f32, canvasSize : vec2f, emitterPos : vec2f, emitterType : f32, emitterP1 : f32, emitterP2 : f32, sp0 : f32, sp1 : f32, sp2 : f32, sp3 : f32, sp4 : f32 } struct Particle { position : vec2f, velocity : vec2f, color : vec4f, seed : f32, life : f32, maxLife : f32, size : f32, } struct VertexOut { @builtin(position) pos : vec4f, @location(0) uv : vec2f, @location(1) color : vec4f, } @group(0) @binding(0) var<uniform> uniforms : Uniforms; @group(0) @binding(1) var<storage, read> particles : array<Particle>; const QUAD = array<vec2f, 6>( vec2f(-1.0, -1.0), vec2f( 1.0, -1.0), vec2f(-1.0, 1.0), vec2f(-1.0, 1.0), vec2f( 1.0, -1.0), vec2f( 1.0, 1.0), ); @vertex fn vs_main( @builtin(vertex_index) vertIdx : u32, @builtin(instance_index) instIdx : u32, ) -> VertexOut { let p = particles[instIdx]; let corner = QUAD[vertIdx]; let pixelPos = p.position + corner * p.size; let clipPos = (pixelPos / uniforms.canvasSize) * 2.0 - 1.0; var out: VertexOut; out.pos = vec4f(clipPos.x, -clipPos.y, 0.0, 1.0); out.uv = corner; out.color = p.color; return out; }";

// src/internal/shaders/scatter-swirl/scatter-swirl.comp.wgsl
var scatter_swirl_comp_default = "struct Uniforms { deltaTime : f32, time : f32, canvasSize : vec2f, emitterPos : vec2f, emitterType : f32, emitterP1 : f32, emitterP2 : f32, sp0 : f32, sp1 : f32, sp2 : f32, sp3 : f32, sp4 : f32, } struct Particle { position : vec2f, velocity : vec2f, color : vec4f, seed : f32, life : f32, maxLife : f32, size : f32, } @group(0) @binding(0) var<uniform> uniforms : Uniforms; @group(0) @binding(1) var<storage, read_write> particles : array<Particle>; fn pcg(v: u32) -> u32 { let state = v * 747796405u + 2891336453u; let word = ((state >> ((state >> 28u) + 4u)) ^ state) * 277803737u; return (word >> 22u) ^ word; } fn rand_f32(seed: u32) -> f32 { return f32(pcg(seed)) / f32(0xffffffffu); } fn respawn(i: u32, seed: f32) -> Particle { var p: Particle; let s = u32(seed * f32(0xffffffffu)); let r = rand_f32(pcg(i ^ s)) * uniforms.emitterP1; let angle = rand_f32(pcg((i ^ s) + 100000u)) * 6.2831853; p.position = uniforms.emitterPos + vec2f(cos(angle), sin(angle)) * r; p.velocity = vec2f(0.0); p.seed = rand_f32(pcg((i ^ s) + 200000u)); p.life = 1.0; p.maxLife = rand_f32(pcg((i ^ s) + 300000u)) * 2.0 + 5.0; p.color = vec4f(1.0); p.size = 20.0; return p; } @compute @workgroup_size(64) fn main(@builtin(global_invocation_id) id: vec3u) { let i = id.x; if (i >= arrayLength(&particles)) { return; } var p = particles[i]; p.life -= uniforms.deltaTime / p.maxLife; if (p.life <= 0.0) { particles[i] = respawn(i, p.seed); return; } let toCenter = uniforms.emitterPos - p.position; let dist = max(length(toCenter), 1.0); let radial = toCenter / dist; let tangent = vec2f(-radial.y, radial.x); let spin = tangent * uniforms.sp0 / sqrt(dist); let pull = radial * uniforms.sp1; p.velocity += (spin + pull) * uniforms.deltaTime; p.position += p.velocity * uniforms.deltaTime; p.color.a = p.life; particles[i] = p; }";

// src/internal/shaders/fireworks/fireworks.frag.wgsl
var fireworks_frag_default = "struct VertexOut { @builtin(position) pos : vec4f, @location(0) uv : vec2f, @location(1) color : vec4f, } @fragment fn fs_main(in: VertexOut) -> @location(0) vec4f { let isSpark = in.uv.y > 5.0; let uv = select(in.uv, vec2f(in.uv.x, in.uv.y - 10.0), isSpark); if (!isSpark) { let d = length(uv); if (d > 1.0) { discard; } let core = 1.0 - smoothstep(0.0, 0.5, d); let glow = pow(core, 2.0); let col = mix(vec3f(1.0, 0.8, 0.2), vec3f(1.0, 1.0, 1.0), core); return vec4f(col * glow, glow); } else { let crossSection = 1.0 - smoothstep(0.0, 1.0, abs(uv.x)); let tip = 1.0 - smoothstep(0.0, 0.6, uv.y); let tail = 1.0 - smoothstep(-1.0, 0.2, uv.y); let along = tip * tail; let intensity = crossSection * along; if (intensity < 0.01) { discard; } let hotColor = vec3f(1.0, 1.0, 0.9); let col = mix(in.color.rgb, hotColor, pow(intensity, 3.0)); return vec4f(col * intensity, intensity * in.color.a); } }";

// src/internal/shaders/fireworks/fireworks.vert.wgsl
var fireworks_vert_default = "struct Uniforms { deltaTime : f32, time : f32, canvasSize : vec2f, emitterPos : vec2f, emitterType : f32, emitterP1 : f32, emitterP2 : f32, sp0 : f32, sp1 : f32, sp2 : f32, sp3 : f32, sp4 : f32, } struct Particle { position : vec2f, velocity : vec2f, color : vec4f, seed : f32, life : f32, maxLife : f32, size : f32, } struct VertexOut { @builtin(position) pos : vec4f, @location(0) uv : vec2f, @location(1) color : vec4f, } @group(0) @binding(0) var<uniform> uniforms : Uniforms; @group(0) @binding(1) var<storage, read> particles : array<Particle>; const QUAD = array<vec2f, 6>( vec2f(-1.0, -1.0), vec2f( 1.0, -1.0), vec2f(-1.0, 1.0), vec2f(-1.0, 1.0), vec2f( 1.0, -1.0), vec2f( 1.0, 1.0), ); @vertex fn vs_main( @builtin(vertex_index) vertIdx : u32, @builtin(instance_index) instIdx : u32, ) -> VertexOut { let p = particles[instIdx]; let corner = QUAD[vertIdx]; var pixelPos: vec2f; if (p.seed < 0.5) { pixelPos = p.position + corner * p.size; } else { let speed = length(p.velocity); if (speed > 0.001) { let along = normalize(p.velocity); let perp = vec2f(-along.y, along.x); let stretchLen = p.size + speed * 0.04; let widthLen = p.size * 0.4; pixelPos = p.position + along * corner.y * stretchLen + perp * corner.x * widthLen; } else { pixelPos = p.position + corner * p.size; } } let clipPos = (pixelPos / uniforms.canvasSize) * 2.0 - 1.0; var out: VertexOut; out.pos = vec4f(clipPos.x, -clipPos.y, 0.0, 1.0); out.color = p.color; if (p.seed < 0.5) { out.uv = corner; } else { out.uv = vec2f(corner.x, corner.y + 10.0); } return out; }";

// src/internal/shaders/fireworks/fireworks.comp.wgsl
var fireworks_comp_default = "struct Uniforms { deltaTime : f32, time : f32, canvasSize : vec2f, emitterPos : vec2f, emitterType : f32, emitterP1 : f32, emitterP2 : f32, sp0 : f32, sp1 : f32, sp2 : f32, sp3 : f32, sp4 : f32, } struct Particle { position : vec2f, velocity : vec2f, color : vec4f, seed : f32, life : f32, maxLife : f32, size : f32, } @group(0) @binding(0) var<uniform> uniforms : Uniforms; @group(0) @binding(1) var<storage, read_write> particles : array<Particle>; fn pcg(v: u32) -> u32 { let state = v * 747796405u + 2891336453u; let word = ((state >> ((state >> 28u) + 4u)) ^ state) * 277803737u; return (word >> 22u) ^ word; } fn rand_f32(seed: u32) -> f32 { return f32(pcg(seed)) / f32(0xffffffffu); } fn dormantSpark() -> Particle { var p: Particle; p.position = vec2f(-99999.0); p.velocity = vec2f(0.0); p.color = vec4f(0.0); p.seed = 1.0; p.life = 0.0; p.maxLife = 1.0; p.size = 0.0; return p; } fn respawnRocket(i: u32) -> Particle { var p: Particle; let r1 = rand_f32(pcg(i)); let r2 = rand_f32(pcg(i + 100000u)); let r3 = rand_f32(pcg(i + 200000u)); let r4 = rand_f32(pcg(i + 300000u)); let r5 = rand_f32(pcg(i + 400000u)); let angle = (r2 - 0.5) * 0.4; let mod_speed = (0.8 + (r2 * 0.2)) * -uniforms.sp0; p.position = uniforms.emitterPos + vec2f((r1 - 0.5) * uniforms.emitterP1, uniforms.emitterP2 / 2); p.velocity = vec2f(sin(angle) * uniforms.sp0 * 0.3, mod_speed); p.color = vec4f(r3, r4, r5, 1.0); p.seed = 0.0; p.life = 1.0; p.maxLife = 1.0; p.size = 15.0; return p; } @compute @workgroup_size(64) fn main_rockets(@builtin(global_invocation_id) id: vec3u) { let i = id.x; let rocketCount = u32(uniforms.sp3); if (i >= rocketCount) { return; } var p = particles[i]; if (p.seed == 0.25) { particles[i] = respawnRocket(i); return; } p.velocity.y += uniforms.sp2 * uniforms.deltaTime; p.position += p.velocity * uniforms.deltaTime; if (p.velocity.y >= 0.0) { p.seed = 0.25; } particles[i] = p; } @compute @workgroup_size(64) fn main_sparks(@builtin(global_invocation_id) id: vec3u) { let i = id.x; let rocketCount = u32(uniforms.sp3); let totalParticles = arrayLength(&particles); let r1 = rand_f32(pcg(i)); let r2 = rand_f32(pcg(i + 100000u)); let r3 = rand_f32(pcg(i + 200000u)); let r4 = rand_f32(pcg(i + 300000u)); if (i < rocketCount || i >= totalParticles) { return; } if (rocketCount == 0u) { return; } let sparksPerRocket = (totalParticles - rocketCount) / rocketCount; if (sparksPerRocket == 0u) { return; } var p = particles[i]; let rocketIdx = (i - rocketCount) / sparksPerRocket; if (rocketIdx >= rocketCount) { return; } let rocket = particles[rocketIdx]; if (rocket.seed == 0.25 && p.size == 0.0) { let s = i; let angle = r1 * 6.2831853; let speed = r2 * uniforms.sp1 + uniforms.sp1 * 0.3; p.position = rocket.position; p.velocity = vec2f(cos(angle), sin(angle)) * speed; p.color = vec4f(rocket.color.rgb, 1.0); p.seed = 1.0; p.life = 1.0; p.maxLife = r3 * 1.5 + 0.5; p.size = r4 * 9.0 + 6.0; particles[i] = p; return; } if (p.size == 0.0) { return; } p.life -= uniforms.deltaTime / p.maxLife; if (p.life <= 0.0) { particles[i] = dormantSpark(); return; } p.velocity *= 0.98; p.velocity.y += uniforms.sp2 * uniforms.deltaTime; p.position += p.velocity * uniforms.deltaTime; p.color.a = p.life; particles[i] = p; }";

// src/internal/shaders/boids/boids.frag.wgsl
var boids_frag_default = "struct FragmentInput { @location(0) color : vec4f, } @fragment fn fs_main(input: FragmentInput) -> @location(0) vec4f { return input.color; }";

// src/internal/shaders/boids/boids.vert.wgsl
var boids_vert_default = "struct Uniforms { deltaTime : f32, time : f32, canvasSize : vec2f, emitterPos : vec2f, emitterType : f32, emitterP1 : f32, emitterP2 : f32, sp0 : f32, sp1 : f32, sp2 : f32, sp3 : f32, sp4 : f32, sp5 : f32, sp6 : f32, sp7 : f32, sp8 : f32, sp9 : f32, sp10 : f32, } struct Particle { position : vec2f, velocity : vec2f, color : vec4f, seed : f32, life : f32, maxLife : f32, size : f32, } struct VertexOutput { @builtin(position) position : vec4f, @location(0) color : vec4f, } @group(0) @binding(0) var<uniform> uniforms : Uniforms; @group(0) @binding(1) var<storage, read> particles : array<Particle>; const OFFSETS = array<vec2f, 3>( vec2f( 0.0, 1.0), vec2f( 0.5, -0.6), vec2f(-0.5, -0.6), ); @vertex fn vs_main( @builtin(vertex_index) vertexIndex : u32, @builtin(instance_index) instanceIndex : u32, ) -> VertexOutput { let p = particles[instanceIndex]; let speed = length(p.velocity); var forward = vec2f(0.0, -1.0); if (speed > 0.5) { forward = p.velocity / speed; } let right = vec2f(forward.y, -forward.x); let local = OFFSETS[vertexIndex]; let pixelPos = p.position + forward * local.y * p.size + right * local.x * p.size; let clip = (pixelPos / uniforms.canvasSize) * 2.0 - 1.0; var output: VertexOutput; output.position = vec4f(clip.x, -clip.y, 0.0, 1.0); output.color = p.color; return output; }";

// src/internal/shaders/boids/boids.comp.wgsl
var boids_comp_default = "struct Uniforms { deltaTime : f32, time : f32, canvasSize : vec2f, emitterPos : vec2f, emitterType : f32, emitterP1 : f32, emitterP2 : f32, sp0 : f32, sp1 : f32, sp2 : f32, sp3 : f32, sp4 : f32, sp5 : f32, sp6 : f32, sp7 : f32, sp8 : f32, sp9 : f32, sp10 : f32, } struct Particle { position : vec2f, velocity : vec2f, color : vec4f, seed : f32, life : f32, maxLife : f32, size : f32, } @group(0) @binding(0) var<uniform> uniforms : Uniforms; @group(0) @binding(1) var<storage, read> particlesIn : array<Particle>; @group(0) @binding(2) var<storage, read_write> particlesOut : array<Particle>; fn clampMagnitude(v: vec2f, maxMag: f32) -> vec2f { let mag = length(v); if (mag > maxMag && mag > 0.0) { return (v / mag) * maxMag; } return v; } @compute @workgroup_size(64) fn main(@builtin(global_invocation_id) id: vec3u) { let count = arrayLength(&particlesIn); let index = id.x; if (index >= count) { return; } var p = particlesIn[index]; let perceptionRadius = uniforms.sp0; let separationRadius = uniforms.sp1; let separationWeight = uniforms.sp2; let alignmentWeight = uniforms.sp3; let cohesionWeight = uniforms.sp4; let maxSpeed = uniforms.sp5; let maxForce = uniforms.sp6; var separation = vec2f(0.0); var alignment = vec2f(0.0); var cohesion = vec2f(0.0); var neighborCount = 0u; var separationCount = 0u; for (var i = 0u; i < count; i++) { if (i == index) { continue; } let other = particlesIn[i]; let diff = p.position - other.position; let dist = length(diff); if (dist < perceptionRadius && dist > 0.0) { neighborCount++; alignment += other.velocity; cohesion += other.position; if (dist < separationRadius) { separationCount++; let safeDist = max(dist, 1.0); separation += normalize(diff) / safeDist; } } } var steer = vec2f(0.0); if (neighborCount > 0u) { let n = f32(neighborCount); let avgVelocity = alignment / n; let desiredAlign = normalize(avgVelocity) * maxSpeed; steer += clampMagnitude((desiredAlign - p.velocity) * alignmentWeight, maxForce); let centerOfMass = cohesion / n; let toCOM = centerOfMass - p.position; let distToCOM = length(toCOM); if (distToCOM > 0.0) { let desiredCohesion = normalize(toCOM) * maxSpeed; steer += clampMagnitude((desiredCohesion - p.velocity) * cohesionWeight, maxForce); } } if (separationCount > 0u) { let sepDir = separation / f32(separationCount); let desiredSep = normalize(sepDir) * maxSpeed; steer += clampMagnitude((desiredSep - p.velocity) * separationWeight, maxForce); } p.velocity += steer * uniforms.deltaTime; p.velocity = clampMagnitude(p.velocity, maxSpeed); p.position += p.velocity * uniforms.deltaTime; p.position.x = ((p.position.x % uniforms.canvasSize.x) + uniforms.canvasSize.x) % uniforms.canvasSize.x; p.position.y = ((p.position.y % uniforms.canvasSize.y) + uniforms.canvasSize.y) % uniforms.canvasSize.y; particlesOut[index] = p; }";

// src/internal/shaders/index.ts
var shader_registry = {
  scatter_fade_frag: scatter_fade_frag_default,
  scatter_fade_vert: scatter_fade_vert_default,
  scatter_fade_comp: scatter_fade_comp_default,
  scatter_swirl_frag: scatter_swirl_frag_default,
  scatter_swirl_vert: scatter_swirl_vert_default,
  scatter_swirl_comp: scatter_swirl_comp_default,
  fireworks_frag: fireworks_frag_default,
  fireworks_vert: fireworks_vert_default,
  fireworks_comp: fireworks_comp_default,
  boids_frag: boids_frag_default,
  boids_vert: boids_vert_default,
  boids_comp: boids_comp_default
};

// src/internal/EffectRenderer/ScatterFadeRenderer.ts
var ScatterFadeRenderer = class _ScatterFadeRenderer {
  compute_pipelines = {};
  render_pipelines = {};
  buffers = {};
  bindgroups = {};
  shaders = {};
  max_particles;
  particle_stride = 32;
  // standard particle size
  workgroup_size = 64;
  // standard workgroup size
  constructor(effect) {
    this.max_particles = effect.get_max_particles();
    effect.seed_particles();
  }
  static initialize(effect) {
    let result = new _ScatterFadeRenderer(effect);
    return result;
  }
  async create_shaders(device) {
    let results = {};
    let shaders_text = this.load_shaders();
    let shaders_compiled = {};
    shaders_compiled["compute"] = device.createShaderModule({
      code: shaders_text["compute"],
      label: "particle_compute_shader"
    });
    await this.log_shader_errors(shaders_compiled["compute"]);
    shaders_compiled["vert"] = device.createShaderModule({
      code: shaders_text["vert"],
      label: "particle_vert_shader"
    });
    await this.log_shader_errors(shaders_compiled["vert"]);
    shaders_compiled["frag"] = device.createShaderModule({
      code: shaders_text["frag"],
      label: "particle_frag_shader"
    });
    await this.log_shader_errors(shaders_compiled["frag"]);
    this.shaders = shaders_compiled;
  }
  create_buffers(device) {
    let buffers = {};
    buffers["particle_buffer_0"] = device.createBuffer({
      label: "particles",
      size: this.max_particles * this.particle_stride,
      usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST
    });
    this.buffers = buffers;
  }
  create_compute_pipelines(device) {
    let results = {};
    results["compute_pipeline"] = device.createComputePipeline({
      label: "compute_pipeline",
      layout: "auto",
      compute: { module: this.shaders["compute"], entryPoint: "main" }
    });
    this.compute_pipelines = results;
  }
  create_render_pipelines(device) {
    let results = {};
    results["render_pipeline"] = device.createRenderPipeline({
      vertex: {
        module: this.shaders["vert"],
        entryPoint: "vs_main"
      },
      fragment: {
        module: this.shaders["frag"],
        entryPoint: "fs_main",
        targets: [
          {
            format: navigator.gpu.getPreferredCanvasFormat(),
            blend: {
              color: {
                srcFactor: "src-alpha",
                dstFactor: "one-minus-src-alpha",
                operation: "add"
              },
              alpha: {
                srcFactor: "one",
                dstFactor: "one-minus-src-alpha",
                operation: "add"
              }
            }
          }
        ]
      },
      primitive: {
        topology: "triangle-list"
      },
      layout: "auto"
    });
    this.render_pipelines = results;
  }
  create_bindgroups(device, uniform_buffers) {
    this.bindgroups["compute_bind_group"] = device.createBindGroup({
      layout: this.compute_pipelines["compute_pipeline"].getBindGroupLayout(0),
      entries: [
        { binding: 0, resource: { buffer: uniform_buffers["uniform_buffer"] } },
        { binding: 1, resource: { buffer: this.buffers["particle_buffer_0"] } }
      ]
    });
    this.bindgroups["render_bind_group"] = device.createBindGroup({
      layout: this.render_pipelines["render_pipeline"].getBindGroupLayout(0),
      entries: [
        { binding: 0, resource: { buffer: uniform_buffers["uniform_buffer"] } },
        { binding: 1, resource: { buffer: this.buffers["particle_buffer_0"] } }
      ]
    });
  }
  write_particles_to_buffer(device, emitter_config) {
    let data_array = new Float32Array(
      this.max_particles * (this.particle_stride / 4)
    );
    device.queue.writeBuffer(this.buffers["particle_buffer_0"], 0, data_array);
  }
  destroy() {
  }
  encode_compute_pass(encoder) {
    let compute = encoder.beginComputePass();
    compute.setPipeline(this.compute_pipelines["compute_pipeline"]);
    compute.setBindGroup(0, this.bindgroups["compute_bind_group"]);
    compute.dispatchWorkgroups(
      Math.ceil(this.max_particles / this.workgroup_size)
    );
    compute.end();
  }
  encode_render_pass(encoder, view) {
    const render = encoder.beginRenderPass({
      colorAttachments: [
        {
          view,
          clearValue: { r: 0, g: 0, b: 0, a: 1 },
          loadOp: "clear",
          storeOp: "store"
        }
      ]
    });
    render.setPipeline(this.render_pipelines["render_pipeline"]);
    render.setBindGroup(0, this.bindgroups["render_bind_group"]);
    render.draw(6, this.max_particles);
    render.end();
  }
  // #region --- Helper functions ----------------------------------------------
  /**
   *
   * Dynamically load the particle engine shaders
   *
   * @param name - The name for the shader set
   * @returns the shaders
   */
  load_shaders() {
    var comp_s = `scatter_fade_comp`;
    var vert_s = `scatter_fade_vert`;
    var frag_s = `scatter_fade_frag`;
    const comp_key = comp_s;
    const vert_key = vert_s;
    const frag_key = frag_s;
    const compute = shader_registry[comp_key];
    const vert = shader_registry[vert_key];
    const frag = shader_registry[frag_key];
    if (!compute || !vert || !frag) {
      throw new Error(
        `No shaders found for "${name}". Available: ${Object.keys(shader_registry).join(", ")}`
      );
    }
    return { compute, vert, frag };
  }
  /**
   * Log any errors reported during shader creation
   *
   * @param shader - The shader to do error reporting for
   */
  async log_shader_errors(shader) {
    const info = await shader.getCompilationInfo();
    for (const msg of info.messages) {
      if (msg.type === "error") {
        console.error(`shader error: ${msg.message} (line ${msg.lineNum})`);
      }
    }
  }
  // #endregion ----------------------------------------------------------------
};

// src/internal/EffectRenderer/FireworksRenderer.ts
var FireworksRenderer = class _FireworksRenderer {
  compute_pipelines = {};
  render_pipelines = {};
  buffers = {};
  bindgroups = {};
  shaders = {};
  max_particles;
  particle_stride = 32;
  // standard particle size
  workgroup_size = 64;
  // standard workgroup size
  constructor(effect) {
    this.max_particles = effect.get_max_particles();
    effect.seed_particles();
  }
  static initialize(effect) {
    let result = new _FireworksRenderer(effect);
    return result;
  }
  async create_shaders(device) {
    let shaders_text = this.load_shaders();
    let shaders_compiled = {};
    shaders_compiled["compute"] = device.createShaderModule({
      code: shaders_text["compute"],
      label: "particle_compute_shader"
    });
    await this.log_shader_errors(shaders_compiled["compute"]);
    shaders_compiled["vert"] = device.createShaderModule({
      code: shaders_text["vert"],
      label: "particle_vert_shader"
    });
    await this.log_shader_errors(shaders_compiled["vert"]);
    shaders_compiled["frag"] = device.createShaderModule({
      code: shaders_text["frag"],
      label: "particle_frag_shader"
    });
    await this.log_shader_errors(shaders_compiled["frag"]);
    this.shaders = shaders_compiled;
  }
  create_buffers(device) {
    let buffers = {};
    buffers["particle_buffer_0"] = device.createBuffer({
      label: "particles",
      size: this.max_particles * this.particle_stride,
      usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST
    });
    this.buffers = buffers;
  }
  create_compute_pipelines(device) {
    let results = {};
    results["rocket_pipeline"] = device.createComputePipeline({
      label: "rocket_pipeline",
      layout: "auto",
      compute: { module: this.shaders["compute"], entryPoint: "main_rockets" }
    });
    results["spark_pipeline"] = device.createComputePipeline({
      label: "sparks_pipeline",
      layout: "auto",
      compute: { module: this.shaders["compute"], entryPoint: "main_sparks" }
    });
    this.compute_pipelines = results;
  }
  create_render_pipelines(device) {
    let results = {};
    results["render_pipeline"] = device.createRenderPipeline({
      vertex: {
        module: this.shaders["vert"],
        entryPoint: "vs_main"
      },
      fragment: {
        module: this.shaders["frag"],
        entryPoint: "fs_main",
        targets: [
          {
            format: navigator.gpu.getPreferredCanvasFormat(),
            blend: {
              color: {
                srcFactor: "src-alpha",
                dstFactor: "one-minus-src-alpha",
                operation: "add"
              },
              alpha: {
                srcFactor: "one",
                dstFactor: "one-minus-src-alpha",
                operation: "add"
              }
            }
          }
        ]
      },
      primitive: {
        topology: "triangle-list"
      },
      layout: "auto"
    });
    this.render_pipelines = results;
  }
  create_bindgroups(device, uniform_buffers) {
    this.bindgroups["rocket_bind_group"] = device.createBindGroup({
      layout: this.compute_pipelines["rocket_pipeline"].getBindGroupLayout(0),
      entries: [
        { binding: 0, resource: { buffer: uniform_buffers["uniform_buffer"] } },
        { binding: 1, resource: { buffer: this.buffers["particle_buffer_0"] } }
      ]
    });
    this.bindgroups["spark_bind_group"] = device.createBindGroup({
      layout: this.compute_pipelines["spark_pipeline"].getBindGroupLayout(0),
      entries: [
        { binding: 0, resource: { buffer: uniform_buffers["uniform_buffer"] } },
        { binding: 1, resource: { buffer: this.buffers["particle_buffer_0"] } }
      ]
    });
    this.bindgroups["render_bind_group"] = device.createBindGroup({
      layout: this.render_pipelines["render_pipeline"].getBindGroupLayout(0),
      entries: [
        { binding: 0, resource: { buffer: uniform_buffers["uniform_buffer"] } },
        { binding: 1, resource: { buffer: this.buffers["particle_buffer_0"] } }
      ]
    });
  }
  write_particles_to_buffer(device, emitter_config) {
    let data_array = new Float32Array(
      this.max_particles * (this.particle_stride / 4)
    );
    device.queue.writeBuffer(this.buffers["particle_buffer_0"], 0, data_array);
  }
  destroy() {
  }
  encode_compute_pass(encoder) {
    let compute = encoder.beginComputePass();
    compute.setPipeline(this.compute_pipelines["rocket_pipeline"]);
    compute.setBindGroup(0, this.bindgroups["rocket_bind_group"]);
    compute.dispatchWorkgroups(
      Math.ceil(this.max_particles / this.workgroup_size)
    );
    compute.setPipeline(this.compute_pipelines["spark_pipeline"]);
    compute.setBindGroup(0, this.bindgroups["spark_bind_group"]);
    compute.dispatchWorkgroups(
      Math.ceil(this.max_particles / this.workgroup_size)
    );
    compute.end();
  }
  encode_render_pass(encoder, view) {
    const render = encoder.beginRenderPass({
      colorAttachments: [
        {
          view,
          clearValue: { r: 0, g: 0, b: 0, a: 1 },
          loadOp: "clear",
          storeOp: "store"
        }
      ]
    });
    render.setPipeline(this.render_pipelines["render_pipeline"]);
    render.setBindGroup(0, this.bindgroups["render_bind_group"]);
    render.draw(6, this.max_particles);
    render.end();
  }
  // #region --- Helper functions ----------------------------------------------
  /**
   *
   * Dynamically load the particle engine shaders
   *
   * @param name - The name for the shader set
   * @returns the shaders
   */
  load_shaders() {
    var comp_s = `fireworks_comp`;
    var vert_s = `fireworks_vert`;
    var frag_s = `fireworks_frag`;
    const comp_key = comp_s;
    const vert_key = vert_s;
    const frag_key = frag_s;
    const compute = shader_registry[comp_key];
    const vert = shader_registry[vert_key];
    const frag = shader_registry[frag_key];
    if (!compute || !vert || !frag) {
      throw new Error(
        `No shaders found for "${name}". Available: ${Object.keys(shader_registry).join(", ")}`
      );
    }
    return { compute, vert, frag };
  }
  /**
   * Log any errors reported during shader creation
   *
   * @param shader - The shader to do error reporting for
   */
  async log_shader_errors(shader) {
    const info = await shader.getCompilationInfo();
    for (const msg of info.messages) {
      if (msg.type === "error") {
        console.error(`shader error: ${msg.message} (line ${msg.lineNum})`);
      }
    }
  }
  // #endregion ----------------------------------------------------------------
};

// src/public/enums/EmitterShapes.ts
var EmitterShape = /* @__PURE__ */ ((EmitterShape2) => {
  EmitterShape2[EmitterShape2["Point"] = 0] = "Point";
  EmitterShape2[EmitterShape2["Circle"] = 1] = "Circle";
  EmitterShape2[EmitterShape2["Rectangle"] = 2] = "Rectangle";
  return EmitterShape2;
})(EmitterShape || {});

// src/internal/EffectRenderer/BoidsRenderer.ts
var BoidsRenderer = class _BoidsRenderer {
  compute_pipelines = {};
  render_pipelines = {};
  buffers = {};
  bindgroups = {};
  shaders = {};
  frame_index = 0;
  max_particles;
  particle_stride = 32;
  // standard particle size
  workgroup_size = 64;
  // standard workgroup size
  constructor(effect) {
    this.max_particles = effect.get_max_particles();
    effect.seed_particles();
  }
  static initialize(effect) {
    let result = new _BoidsRenderer(effect);
    return result;
  }
  async create_shaders(device) {
    let shaders_text = this.load_shaders();
    let shaders_compiled = {};
    shaders_compiled["compute"] = device.createShaderModule({
      code: shaders_text["compute"],
      label: "particle_compute_shader"
    });
    await this.log_shader_errors(shaders_compiled["compute"]);
    shaders_compiled["vert"] = device.createShaderModule({
      code: shaders_text["vert"],
      label: "particle_vert_shader"
    });
    await this.log_shader_errors(shaders_compiled["vert"]);
    shaders_compiled["frag"] = device.createShaderModule({
      code: shaders_text["frag"],
      label: "particle_frag_shader"
    });
    await this.log_shader_errors(shaders_compiled["frag"]);
    this.shaders = shaders_compiled;
  }
  create_buffers(device) {
    let buffers = {};
    buffers["particle_buffer_0"] = device.createBuffer({
      label: "particles-0",
      size: this.max_particles * this.particle_stride,
      usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST
    });
    buffers["particle_buffer_1"] = device.createBuffer({
      label: "particles-1",
      size: this.max_particles * this.particle_stride,
      usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST
    });
    this.buffers = buffers;
  }
  create_compute_pipelines(device) {
    let results = {};
    results["compute_pipeline"] = device.createComputePipeline({
      label: "compute_pipeline",
      layout: "auto",
      compute: { module: this.shaders["compute"], entryPoint: "main" }
    });
    this.compute_pipelines = results;
  }
  create_render_pipelines(device) {
    let results = {};
    results["render_pipeline"] = device.createRenderPipeline({
      vertex: {
        module: this.shaders["vert"],
        entryPoint: "vs_main"
      },
      fragment: {
        module: this.shaders["frag"],
        entryPoint: "fs_main",
        targets: [
          {
            format: navigator.gpu.getPreferredCanvasFormat(),
            blend: {
              color: {
                srcFactor: "src-alpha",
                dstFactor: "one-minus-src-alpha",
                operation: "add"
              },
              alpha: {
                srcFactor: "one",
                dstFactor: "one-minus-src-alpha",
                operation: "add"
              }
            }
          }
        ]
      },
      primitive: {
        topology: "triangle-list"
      },
      layout: "auto"
    });
    this.render_pipelines = results;
  }
  create_bindgroups(device, uniform_buffers) {
    this.bindgroups["compute_bind_group_0"] = device.createBindGroup({
      layout: this.compute_pipelines["compute_pipeline"].getBindGroupLayout(0),
      entries: [
        { binding: 0, resource: { buffer: uniform_buffers["uniform_buffer"] } },
        // write
        { binding: 1, resource: { buffer: this.buffers["particle_buffer_0"] } },
        { binding: 2, resource: { buffer: this.buffers["particle_buffer_1"] } }
        // read
      ]
    });
    this.bindgroups["compute_bind_group_1"] = device.createBindGroup({
      layout: this.compute_pipelines["compute_pipeline"].getBindGroupLayout(0),
      entries: [
        { binding: 0, resource: { buffer: uniform_buffers["uniform_buffer"] } },
        // read
        { binding: 1, resource: { buffer: this.buffers["particle_buffer_1"] } },
        // write
        { binding: 2, resource: { buffer: this.buffers["particle_buffer_0"] } }
      ]
    });
    this.bindgroups["render_bind_group_0"] = device.createBindGroup({
      layout: this.render_pipelines["render_pipeline"].getBindGroupLayout(0),
      entries: [
        { binding: 0, resource: { buffer: uniform_buffers["uniform_buffer"] } },
        { binding: 1, resource: { buffer: this.buffers["particle_buffer_1"] } }
      ]
    });
  }
  write_particles_to_buffer(device, emitter_config) {
    let data = new Float32Array(
      this.max_particles * (this.particle_stride / 4)
    );
    for (let i = 0; i < this.max_particles; i++) {
      const offset = i * 12;
      let x, y;
      switch (emitter_config.shape) {
        case 0 /* Point */:
          x = emitter_config.pos[0];
          y = emitter_config.pos[1];
          break;
        case 1 /* Circle */:
          const angle = Math.random() * Math.PI * 2;
          const radius = Math.sqrt(Math.random()) * (emitter_config.p1 ?? 0);
          x = emitter_config.pos[0] + Math.cos(angle) * radius;
          y = emitter_config.pos[1] + Math.sin(angle) * radius;
          break;
        case 2 /* Rectangle */:
          x = emitter_config.pos[0] + (Math.random() - 0.5) * (emitter_config.p1 ?? 1);
          y = emitter_config.pos[1] + (Math.random() - 0.5) * (emitter_config.p2 ?? 0);
          break;
        default:
          x = 0;
          y = 0;
      }
      data[offset + 0] = x;
      data[offset + 1] = y;
      data[offset + 2] = (Math.random() - 0.5) * 400;
      data[offset + 3] = Math.random() * 300 + 0.2;
      data[offset + 4] = Math.random();
      data[offset + 5] = Math.random();
      data[offset + 6] = Math.random();
      data[offset + 7] = 1;
      data[offset + 8] = 0;
      data[offset + 9] = Math.random();
      data[offset + 10] = 1;
      data[offset + 11] = Math.random() * 8 + 4;
    }
    device.queue.writeBuffer(this.buffers["particle_buffer_0"], 0, data);
  }
  destroy() {
  }
  encode_compute_pass(encoder) {
    let compute = encoder.beginComputePass();
    const bind_group = "compute_bind_group_" + this.frame_index % 2;
    compute.setPipeline(this.compute_pipelines["compute_pipeline"]);
    compute.setBindGroup(0, this.bindgroups[bind_group]);
    compute.dispatchWorkgroups(
      Math.ceil(this.max_particles / this.workgroup_size)
    );
    compute.end();
    this.frame_index++;
  }
  encode_render_pass(encoder, view) {
    const render = encoder.beginRenderPass({
      colorAttachments: [
        {
          view,
          clearValue: { r: 0, g: 0, b: 0, a: 1 },
          loadOp: "clear",
          storeOp: "store"
        }
      ]
    });
    render.setPipeline(this.render_pipelines["render_pipeline"]);
    render.setBindGroup(0, this.bindgroups["render_bind_group_0"]);
    render.draw(6, this.max_particles);
    render.end();
  }
  // #region --- Helper functions ----------------------------------------------
  /**
   *
   * Dynamically load the particle engine shaders
   *
   * @param name - The name for the shader set
   * @returns the shaders
   */
  load_shaders() {
    var comp_s = `boids_comp`;
    var vert_s = `boids_vert`;
    var frag_s = `boids_frag`;
    const comp_key = comp_s;
    const vert_key = vert_s;
    const frag_key = frag_s;
    const compute = shader_registry[comp_key];
    const vert = shader_registry[vert_key];
    const frag = shader_registry[frag_key];
    if (!compute || !vert || !frag) {
      throw new Error(
        `No shaders found for "${name}". Available: ${Object.keys(shader_registry).join(", ")}`
      );
    }
    return { compute, vert, frag };
  }
  /**
   * Log any errors reported during shader creation
   *
   * @param shader - The shader to do error reporting for
   */
  async log_shader_errors(shader) {
    const info = await shader.getCompilationInfo();
    for (const msg of info.messages) {
      if (msg.type === "error") {
        console.error(`shader error: ${msg.message} (line ${msg.lineNum})`);
      }
    }
  }
  // #endregion ----------------------------------------------------------------
};

// src/internal/EffectRenderer/ScatterSwirlRenderer.ts
var ScatterSwirlRenderer = class _ScatterSwirlRenderer {
  compute_pipelines = {};
  render_pipelines = {};
  buffers = {};
  bindgroups = {};
  shaders = {};
  max_particles;
  particle_stride = 32;
  // standard particle size
  workgroup_size = 64;
  // standard workgroup size
  constructor(effect) {
    this.max_particles = effect.get_max_particles();
    effect.seed_particles();
  }
  static initialize(effect) {
    let result = new _ScatterSwirlRenderer(effect);
    return result;
  }
  async create_shaders(device) {
    let shaders_text = this.load_shaders();
    let shaders_compiled = {};
    shaders_compiled["compute"] = device.createShaderModule({
      code: shaders_text["compute"],
      label: "particle_compute_shader"
    });
    await this.log_shader_errors(shaders_compiled["compute"]);
    shaders_compiled["vert"] = device.createShaderModule({
      code: shaders_text["vert"],
      label: "particle_vert_shader"
    });
    await this.log_shader_errors(shaders_compiled["vert"]);
    shaders_compiled["frag"] = device.createShaderModule({
      code: shaders_text["frag"],
      label: "particle_frag_shader"
    });
    await this.log_shader_errors(shaders_compiled["frag"]);
    this.shaders = shaders_compiled;
  }
  create_buffers(device) {
    let buffers = {};
    buffers["particle_buffer_0"] = device.createBuffer({
      label: "particles",
      size: this.max_particles * this.particle_stride,
      usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST
    });
    this.buffers = buffers;
  }
  create_compute_pipelines(device) {
    let results = {};
    results["compute_pipeline"] = device.createComputePipeline({
      label: "compute_pipeline",
      layout: "auto",
      compute: { module: this.shaders["compute"], entryPoint: "main" }
    });
    this.compute_pipelines = results;
  }
  create_render_pipelines(device) {
    let results = {};
    results["render_pipeline"] = device.createRenderPipeline({
      vertex: {
        module: this.shaders["vert"],
        entryPoint: "vs_main"
      },
      fragment: {
        module: this.shaders["frag"],
        entryPoint: "fs_main",
        targets: [
          {
            format: navigator.gpu.getPreferredCanvasFormat(),
            blend: {
              color: {
                srcFactor: "src-alpha",
                dstFactor: "one-minus-src-alpha",
                operation: "add"
              },
              alpha: {
                srcFactor: "one",
                dstFactor: "one-minus-src-alpha",
                operation: "add"
              }
            }
          }
        ]
      },
      primitive: {
        topology: "triangle-list"
      },
      layout: "auto"
    });
    this.render_pipelines = results;
  }
  create_bindgroups(device, uniform_buffers) {
    this.bindgroups["compute_bind_group"] = device.createBindGroup({
      layout: this.compute_pipelines["compute_pipeline"].getBindGroupLayout(0),
      entries: [
        { binding: 0, resource: { buffer: uniform_buffers["uniform_buffer"] } },
        { binding: 1, resource: { buffer: this.buffers["particle_buffer_0"] } }
      ]
    });
    this.bindgroups["render_bind_group"] = device.createBindGroup({
      layout: this.render_pipelines["render_pipeline"].getBindGroupLayout(0),
      entries: [
        { binding: 0, resource: { buffer: uniform_buffers["uniform_buffer"] } },
        { binding: 1, resource: { buffer: this.buffers["particle_buffer_0"] } }
      ]
    });
  }
  write_particles_to_buffer(device, emitter_config) {
    let data_array = new Float32Array(
      this.max_particles * (this.particle_stride / 4)
    );
    device.queue.writeBuffer(this.buffers["particle_buffer_0"], 0, data_array);
  }
  destroy() {
  }
  encode_compute_pass(encoder) {
    let compute = encoder.beginComputePass();
    compute.setPipeline(this.compute_pipelines["compute_pipeline"]);
    compute.setBindGroup(0, this.bindgroups["compute_bind_group"]);
    compute.dispatchWorkgroups(
      Math.ceil(this.max_particles / this.workgroup_size)
    );
    compute.end();
  }
  encode_render_pass(encoder, view) {
    const render = encoder.beginRenderPass({
      colorAttachments: [
        {
          view,
          clearValue: { r: 0, g: 0, b: 0, a: 1 },
          loadOp: "clear",
          storeOp: "store"
        }
      ]
    });
    render.setPipeline(this.render_pipelines["render_pipeline"]);
    render.setBindGroup(0, this.bindgroups["render_bind_group"]);
    render.draw(6, this.max_particles);
    render.end();
  }
  // #region --- Helper functions ----------------------------------------------
  /**
   *
   * Dynamically load the particle engine shaders
   *
   * @param name - The name for the shader set
   * @returns the shaders
   */
  load_shaders() {
    var comp_s = `scatter_swirl_comp`;
    var vert_s = `scatter_swirl_vert`;
    var frag_s = `scatter_swirl_frag`;
    const comp_key = comp_s;
    const vert_key = vert_s;
    const frag_key = frag_s;
    const compute = shader_registry[comp_key];
    const vert = shader_registry[vert_key];
    const frag = shader_registry[frag_key];
    if (!compute || !vert || !frag) {
      throw new Error(
        `No shaders found for "${name}". Available: ${Object.keys(shader_registry).join(", ")}`
      );
    }
    return { compute, vert, frag };
  }
  /**
   * Log any errors reported during shader creation
   *
   * @param shader - The shader to do error reporting for
   */
  async log_shader_errors(shader) {
    const info = await shader.getCompilationInfo();
    for (const msg of info.messages) {
      if (msg.type === "error") {
        console.error(`shader error: ${msg.message} (line ${msg.lineNum})`);
      }
    }
  }
  // #endregion ----------------------------------------------------------------
};

// src/internal/EffectRenderer/EffectRendererFactory.ts
var EffectRendererFactory = class {
  static async initialize(effect, emitter_config, device, uniform_buffers) {
    let result;
    switch (effect.kind) {
      case "BOIDS":
        result = BoidsRenderer.initialize(effect);
        break;
      case "FIREWORKS":
        result = FireworksRenderer.initialize(effect);
        break;
      case "SCATTER_FADE":
        result = ScatterFadeRenderer.initialize(effect);
        break;
      case "SCATTER_SWIRL":
        result = ScatterSwirlRenderer.initialize(effect);
        break;
      default:
        throw new Error(`Unknown effect type: ${effect.kind}`);
    }
    console.log("device before shader compile:", device.label);
    await result.create_shaders(device);
    console.log("device after shader compile:", device.label);
    result.create_buffers(device);
    result.create_compute_pipelines(device);
    result.create_render_pipelines(device);
    result.create_bindgroups(device, uniform_buffers);
    result.write_particles_to_buffer(device, emitter_config);
    return result;
  }
};

// src/public/ParticleEngine.ts
var ParticleEngine = class _ParticleEngine {
  // #region --- The class properties ------------------------------------------
  canvas;
  ctx;
  uniform_values;
  renderer;
  last_time = performance.now();
  paused = false;
  // #endregion ----------------------------------------------------------------
  /**
   *
   * Construct a new particle engine instance.
   * Called by the static init function to enable the async nature of the
   * generation of WebGPU contexts
   *
   * @param ctx - the WebGPUContext created in the init function
   * @param max_particles - the max number of particles requested by the user
   * @param particle_stride - The size of each particle in GPU memory
   */
  constructor(canvas, ctx, emitter_config, effect, renderer) {
    this.canvas = canvas;
    this.ctx = ctx;
    this.uniform_values = this.generate_uniform_values(
      canvas,
      effect,
      emitter_config
    );
    this.renderer = renderer;
  }
  /**
   *
   * Start the animation of the particles
   *
   */
  start() {
    requestAnimationFrame(this.animate_particles);
  }
  pause() {
    this.paused = true;
  }
  resume() {
    this.paused = false;
  }
  /**
   * Update the UniformValues with new emitter values
   *
   * @param emitter_config - The new emitter values to be used
   *
   */
  update_emitter(emitter_config) {
    if (emitter_config.shape !== null) {
      this.uniform_values.emitter_shape = emitter_config.shape ?? this.uniform_values.emitter_shape;
    }
    if (emitter_config.pos !== null) {
      this.uniform_values.emitter_pos[0] = emitter_config.pos[0] ?? this.uniform_values.emitter_pos[0];
      this.uniform_values.emitter_pos[1] = emitter_config.pos[1] ?? this.uniform_values.emitter_pos[1];
    }
    this.uniform_values.emitter_p1 = emitter_config.p1 ?? this.uniform_values.emitter_p1;
    this.uniform_values.emitter_p2 = emitter_config.p2 ?? this.uniform_values.emitter_p1;
  }
  /**
   *
   * Handle each frame of animation tick.
   * Passed to requestAnimationFrame to drive the particle simulation loop.
   * Note use of arrow notation. This keeps this in scope at all times.
   *
   */
  animate_particles = () => {
    const now = performance.now() / 1e3;
    this.uniform_values.delta_time = this.paused ? 0 : now - this.last_time;
    this.uniform_values.time = now;
    this.last_time = now;
    this.ctx.write_buffer(this.canvas, this.uniform_values);
    const encoder = this.ctx.begin_frame();
    this.renderer.encode_compute_pass(encoder);
    this.renderer.encode_render_pass(
      encoder,
      this.ctx.get_current_color_view()
    );
    this.ctx.end_frame(encoder);
    requestAnimationFrame(this.animate_particles);
  };
  /**
   *
   * Check that the context is operating as expected by turning the canvaas red.
   *
   */
  context_check() {
    const encoder = this.ctx.begin_frame();
    const pass = encoder.beginRenderPass(
      this.ctx.create_render_pass_descriptor({ r: 1, g: 0, b: 0, a: 1 })
    );
    pass.end();
    this.ctx.end_frame(encoder);
  }
  /**
   * Update the canvas on resize.
   *
   * @param canvas
   */
  resize(canvas) {
    this.canvas = canvas;
  }
  generate_uniform_values(canvas, effect, emitter_config) {
    const now = performance.now() / 1e3;
    const result = {
      delta_time: now - this.last_time,
      time: now,
      canvas_size: [canvas.width, canvas.height],
      emitter_pos: emitter_config.pos,
      emitter_shape: emitter_config.shape,
      emitter_p1: emitter_config.p1 ?? 0,
      emitter_p2: emitter_config.p2 ?? 0,
      sp0: effect.get_shader_params().sp0,
      sp1: effect.get_shader_params().sp1,
      sp2: effect.get_shader_params().sp2,
      sp3: effect.get_shader_params().sp3,
      sp4: effect.get_shader_params().sp4,
      sp5: effect.get_shader_params().sp5,
      sp6: effect.get_shader_params().sp6,
      sp7: effect.get_shader_params().sp7,
      sp8: effect.get_shader_params().sp8,
      sp9: effect.get_shader_params().sp9,
      sp10: effect.get_shader_params().sp10
    };
    this.last_time = now;
    return result;
  }
  /**
   *
   * Initilize the particle engine in preparation of use
   *
   * @param canvas - the canvas element to tie the particles to.
   * @param max_particles - the number of particles to render.
   * @param emitter_shape - The shape that the particles are emmited as.
   * @param shader_set - the set of shaders to use.
   * @param options - any special options.
   * @returns - The created ParticleEngine
   */
  static async init(canvas, effect, emitter_config, options = {}) {
    var valid_result = effect.validate();
    if (!valid_result.valid) throw new Error(valid_result.message);
    valid_result = emitter_config.validate();
    if (!valid_result.valid) throw new Error(valid_result.message);
    const tmp_ctx = await WebGPUContext.init(canvas, options);
    console.log("context device first:", tmp_ctx.device.label);
    const tmp_renderer = await EffectRendererFactory.initialize(
      effect,
      emitter_config,
      tmp_ctx.device,
      tmp_ctx.get_uniform_buffers()
    );
    console.log("context device second:", tmp_ctx.device.label);
    const result = new _ParticleEngine(
      canvas,
      tmp_ctx,
      emitter_config,
      effect,
      tmp_renderer
    );
    return result;
  }
};

// src/public/enums/ParticleTypes.ts
var ParticleType = /* @__PURE__ */ ((ParticleType2) => {
  ParticleType2[ParticleType2["Star"] = 0] = "Star";
  ParticleType2[ParticleType2["Cross"] = 1] = "Cross";
  ParticleType2[ParticleType2["Square"] = 2] = "Square";
  return ParticleType2;
})(ParticleType || {});

// src/public/types/ValidationRule.ts
function validate(value, rules) {
  for (const rule of rules) {
    if (!rule.check(value)) {
      return { valid: false, message: rule.message };
    }
  }
  return { valid: true };
}

// src/public/effects/ScatterFadeEffect.ts
var ScatterFadeEffect = class _ScatterFadeEffect {
  // #region --- The class properties ------------------------------------------
  kind = "SCATTER_FADE";
  MAX_PARTICLES;
  ping_pong = false;
  static rules = [
    {
      check: (e) => e.MAX_PARTICLES > 0,
      message: "Max Particles must be greater than 0"
    },
    {
      check: (e) => e.MAX_PARTICLES <= 1e5,
      message: "Max particles must not exceed 100,000"
    }
  ];
  // #endregion ----------------------------------------------------------------
  /**
   * The class constructor. Takes the user definable properties and returns an instance of the class
   *
   * @param max_particles - The max number of particles for the effect
   * @param rocket_number - The max number of rockets for the effect
   */
  constructor(max_particles) {
    this.MAX_PARTICLES = max_particles;
  }
  /**
   * Generate all the particles for the intial buffering of the cpu.
   *
   * @returns - An array of particles with random initial values
   *
   */
  seed_particles() {
    return Array.from({ length: this.MAX_PARTICLES }, () => {
      const angle = Math.random() * Math.PI * 2;
      const speed = 100 + Math.random() * 300;
      return {
        position: [0, 0],
        velocity: [Math.cos(angle) * speed, Math.sin(angle) * speed],
        color: [Math.random(), Math.random(), Math.random(), 1],
        seed: Math.random(),
        maxLife: 1 + Math.random() * 2,
        life: 1 + Math.random() * 2,
        size: 2 + Math.random() * 6,
        type: 2 /* Square */
      };
    });
  }
  /**
   * Get the various custom shader particle parameters
   *
   * @returns an object representing the custom shader params.
   */
  get_shader_params() {
    return {
      sp0: 0,
      // - Unused
      sp1: 0,
      // - Unused
      sp2: 0,
      // - Unused
      sp3: 0,
      // - Unused
      sp4: 0,
      // - Unused
      sp5: 0,
      // - Unused
      sp6: 0,
      // - Unused
      sp7: 0,
      // - Unused
      sp8: 0,
      // - Unused
      sp9: 0,
      // - Unused
      sp10: 0
      // - Unused
    };
  }
  /**
   * Getter for the max_particles property.
   *
   * @returns - the maximum number of particles for the effect.
   */
  get_max_particles() {
    return this.MAX_PARTICLES;
  }
  /**
   * Check if the effect uses ping pong buffering or not. This is used by the engine to determine how to buffer the particles on the gpu.
   * @returns - a boolean representing whether the effect uses ping pong buffering or not.
   */
  is_ping_pong() {
    return this.ping_pong;
  }
  /**
   * Validate the object properties
   *
   * @returns
   */
  validate() {
    return validate(this, _ScatterFadeEffect.rules);
  }
};

// src/public/effects/ScatterSwirlEffect.ts
var ScatterSwirlEffect = class _ScatterSwirlEffect {
  // #region --- The class properties ------------------------------------------
  kind = "SCATTER_SWIRL";
  MAX_PARTICLES;
  SPIN_STRENGTH;
  PULL_STRENGTH;
  ping_pong = false;
  static rules = [
    {
      check: (e) => e.MAX_PARTICLES > 0,
      message: "Max Particles must be greater than 0"
    },
    {
      check: (e) => e.MAX_PARTICLES <= 1e5,
      message: "Max particles must not exceed 100,000"
    },
    {
      check: (e) => e.SPIN_STRENGTH > 0,
      message: "Spin strength must be greater than 0"
    },
    {
      check: (e) => e.PULL_STRENGTH > 0,
      message: "Pull strength must be greater than 0"
    }
  ];
  // #endregion ----------------------------------------------------------------
  /**
   * The class constructor. Takes the user definable properties and returns an instance of the class
   *
   * @param max_particles - The max number of particles for the effect
   */
  constructor(max_particles) {
    this.MAX_PARTICLES = max_particles;
    this.SPIN_STRENGTH = 150;
    this.PULL_STRENGTH = 30;
  }
  /**
   * Generate all the particles for the intial buffering of the cpu.
   *
   * @returns - An array of particles with random initial values
   *
   */
  seed_particles() {
    return Array.from({ length: this.MAX_PARTICLES }, () => {
      const angle = Math.random() * Math.PI * 2;
      const speed = 100 + Math.random() * 300;
      return {
        position: [0, 0],
        velocity: [Math.cos(angle) * speed, Math.sin(angle) * speed],
        color: [Math.random(), Math.random(), Math.random(), 1],
        seed: Math.random(),
        maxLife: 1 + Math.random() * 2,
        life: 1 + Math.random() * 2,
        size: 2 + Math.random() * 6,
        type: 2 /* Square */
      };
    });
  }
  /**
   * Get the various custom shader particle parameters
   *
   * @returns an object representing the custom shader params.
   */
  get_shader_params() {
    return {
      sp0: this.SPIN_STRENGTH,
      // - spin strength
      sp1: this.PULL_STRENGTH,
      // - pull strength
      sp2: 0,
      // - Unused
      sp3: 0,
      // - Unused
      sp4: 0,
      // - Unused
      sp5: 0,
      // - Unused
      sp6: 0,
      // - Unused
      sp7: 0,
      // - Unused
      sp8: 0,
      // - Unused
      sp9: 0,
      // - Unused
      sp10: 0
      // - Unused
    };
  }
  /**
   * Getter for the max_particles property.
   *
   * @returns - the maximum number of particles for the effect.
   */
  get_max_particles() {
    return this.MAX_PARTICLES;
  }
  /**
   * Fetch the name of the effect as a string
   *
   */
  get_effect_name() {
    return "scatter-swirl";
  }
  /**
   * Check if the effect uses ping pong buffering or not. This is used by the engine to determine how to buffer the particles on the gpu.
   * @returns - a boolean representing whether the effect uses ping pong buffering or not.
   */
  is_ping_pong() {
    return this.ping_pong;
  }
  /**
   * Validate the object properties
   *
   * @returns
   */
  validate() {
    return validate(this, _ScatterSwirlEffect.rules);
  }
};

// src/public/effects/FireworksEffect.ts
var FireworksEffect = class _FireworksEffect {
  // #region --- The class properties ------------------------------------------
  kind = "FIREWORKS";
  MAX_PARTICLES;
  LAUNCH_SPEED;
  SPARK_SPEED;
  ROCKET_COUNT;
  ping_pong = false;
  static rules = [
    {
      check: (e) => e.MAX_PARTICLES > 0,
      message: "Max Particles must be greater than 0"
    },
    {
      check: (e) => e.MAX_PARTICLES <= 1e5,
      message: "Max particles must not exceed 100,000"
    },
    {
      check: (e) => e.ROCKET_COUNT <= e.MAX_PARTICLES / 2,
      message: "Rocket count must not exceed 1/2 the max particles"
    },
    {
      check: (e) => e.LAUNCH_SPEED > 0,
      message: "Lanuch Speed must be greater than 0"
    }
  ];
  // #endregion ----------------------------------------------------------------
  /**
   * The class constructor. Takes the user definable properties and returns an instance of the class
   *
   * @param max_particles - The max number of particles for the effect
   * @param rocket_number - The max number of rockets for the effect
   */
  constructor(max_particles, rocket_number) {
    this.MAX_PARTICLES = max_particles;
    this.ROCKET_COUNT = rocket_number;
    this.LAUNCH_SPEED = 400;
    this.SPARK_SPEED = 300;
  }
  /**
   * Generate all the particles for the intial buffering of the cpu.
   *
   * @returns - An array of particles with random initial values
   *
   */
  seed_particles() {
    return Array.from({ length: this.ROCKET_COUNT }, () => {
      const angle = Math.random() * Math.PI * 2;
      const speed = 100 + Math.random() * 300;
      return {
        position: [0, 0],
        velocity: [Math.cos(angle) * speed, Math.sin(angle) * speed],
        color: [Math.random(), Math.random(), Math.random(), 1],
        seed: Math.random(),
        maxLife: 1 + Math.random() * 2,
        life: 1 + Math.random() * 2,
        size: 2 + Math.random() * 6,
        type: 2 /* Square */
      };
    });
  }
  /**
   * Get the various custom shader particle parameters
   *
   * @returns an object representing the custom shader params.
   */
  get_shader_params() {
    return {
      sp0: this.LAUNCH_SPEED,
      // - Launch speed
      sp1: this.SPARK_SPEED,
      // - Spark speed
      sp2: 200,
      // - gravity
      sp3: this.ROCKET_COUNT,
      // - Rocket Count
      sp4: 0,
      // - Unused
      sp5: 0,
      // - Unused
      sp6: 0,
      // - Unused
      sp7: 0,
      // - Unused
      sp8: 0,
      // - Unused
      sp9: 0,
      // - Unused
      sp10: 0
      // - Unused
    };
  }
  /**
   * Getter for the max_particles property.
   *
   * @returns - the maximum number of particles for the effect.
   */
  get_max_particles() {
    return this.MAX_PARTICLES;
  }
  /**
   * Fetch the name of the effect as a string
   *
   */
  get_effect_name() {
    return "fireworks";
  }
  /**
   * Check if the effect uses ping pong buffering or not. This is used by the engine to determine how to buffer the particles on the gpu.
   * @returns - a boolean representing whether the effect uses ping pong buffering or not.
   */
  is_ping_pong() {
    return this.ping_pong;
  }
  /**
   * Validate the object properties
   *
   * @returns
   */
  validate() {
    return validate(this, _FireworksEffect.rules);
  }
};

// src/public/effects/BoidsEffect.ts
var BoidsEffect = class _BoidsEffect {
  // #region --- The class properties ------------------------------------------
  kind = "BOIDS";
  MAX_PARTICLES;
  PERCEPTION_RADIUS;
  SEPERATION_RADIUS;
  SEPERATION_WEIGHT;
  ALIGNMENT_WEIGHT;
  COHESION_WEIGHT;
  MAX_SPEED;
  MAX_FORCE;
  ping_pong = true;
  frame_index = 0;
  static rules = [
    {
      check: (e) => e.MAX_PARTICLES > 0,
      message: "Max Particles must be greater than 0"
    },
    {
      check: (e) => e.MAX_PARTICLES <= 1e5,
      message: "Max particles must not exceed 100,000"
    }
  ];
  // #endregion ----------------------------------------------------------------
  /**
   * The class constructor. Takes the user definable properties and returns an instance of the class
   *
   * @param max_particles - The max number of particles for the effect
   * @param rocket_number - The max number of rockets for the effect
   */
  constructor(max_particles, perception_radius, seperation_radius, seperation_weight, alignment_weight, cohesion_weight, max_speed, max_force) {
    this.MAX_PARTICLES = max_particles;
    this.PERCEPTION_RADIUS = perception_radius;
    this.SEPERATION_RADIUS = seperation_radius;
    this.SEPERATION_WEIGHT = seperation_weight;
    this.ALIGNMENT_WEIGHT = alignment_weight;
    this.COHESION_WEIGHT = cohesion_weight;
    this.MAX_SPEED = max_speed;
    this.MAX_FORCE = max_force;
  }
  /**
   * Generate all the particles for the intial buffering of the cpu.
   *
   * @returns - An array of particles with random initial values
   *
   */
  seed_particles() {
    return Array.from({ length: this.MAX_PARTICLES }, () => {
      const angle = Math.random() * Math.PI * 2;
      const speed = 100 + Math.random() * 300;
      return {
        position: [0, 0],
        velocity: [Math.cos(angle) * speed, Math.sin(angle) * speed],
        color: [Math.random(), Math.random(), Math.random(), 1],
        seed: Math.random(),
        maxLife: 1 + Math.random() * 2,
        life: 1 + Math.random() * 2,
        size: 2 + Math.random() * 6,
        type: 2 /* Square */
      };
    });
  }
  /**
   * Get the various custom shader particle parameters
   *
   * @returns an object representing the custom shader params.
   */
  get_shader_params() {
    return {
      sp0: this.PERCEPTION_RADIUS,
      // - Perception Radius
      sp1: this.SEPERATION_RADIUS,
      // - Separation radius
      sp2: this.SEPERATION_WEIGHT,
      // - Separation weight
      sp3: this.ALIGNMENT_WEIGHT,
      // - Alignment weight
      sp4: this.COHESION_WEIGHT,
      // - Cohesion weight
      sp5: this.MAX_SPEED,
      // - Max speed
      sp6: this.MAX_FORCE,
      // - Max force
      sp7: 0,
      // - Unused
      sp8: 0,
      // - Unused
      sp9: 0,
      // - Unused
      sp10: 0
      // - Unused
    };
  }
  /**
   * Getter for the max_particles property.
   *
   * @returns - the maximum number of particles for the effect.
   */
  get_max_particles() {
    return this.MAX_PARTICLES;
  }
  /**
   * Fetch the name of the effect as a string
   *
   */
  get_effect_name() {
    return "boids";
  }
  get_frame_index() {
    return this.frame_index;
  }
  increment_frame_index() {
    this.frame_index = this.frame_index + 1;
  }
  /**
   * Check if the effect uses ping pong buffering or not. This is used by the engine to determine how to buffer the particles on the gpu.
   * @returns - a boolean representing whether the effect uses ping pong buffering or not.
   */
  is_ping_pong() {
    return this.ping_pong;
  }
  /**
   * Validate the object properties
   *
   * @returns
   */
  validate() {
    return validate(this, _BoidsEffect.rules);
  }
};

// src/public/EmitterConfigPatch.ts
var EmitterConfigPatch = class _EmitterConfigPatch {
  // #region --- The class properties -----------------------------------------
  shape;
  pos;
  p1;
  p2;
  static rules = [
    {
      check: (e) => true,
      message: "This is a dummy validation rule that should never trigger. TODO replace with proper validation check"
    }
  ];
  /**
   * The class constructor. Takes the user definable properties and returns an instance of the class
   *
   * @param emitter_type - the type of the emitter
   * @param emitter_pos - the position of the emitter
   * @param emitter_p1 - Custom variable 1 for the emitter. Purpose depends on the shape.
   * @param emitter_p2 - Custom variable 2 for the emitter. Purpose depends on the shape.
   */
  constructor(shape = null, pos = null, p1 = null, p2 = null) {
    this.shape = shape;
    this.pos = pos;
    this.p1 = p1;
    this.p2 = p2;
  }
  /**
   * Validate the object properties
   *
   * @returns
   */
  validate() {
    return validate(this, _EmitterConfigPatch.rules);
  }
};

// src/public/EmitterConfig.ts
var EmitterConfig = class _EmitterConfig extends EmitterConfigPatch {
  // #region --- The class properties -----------------------------------------
  //
  shape;
  pos;
  static rules = [
    {
      check: (e) => true,
      message: "This is a dummy validation rule that should never trigger. TODO replace with proper validation check"
    }
  ];
  // #endregion ---------------------------------------------------------------
  /**
   * The class constructor. Takes the user definable properties and returns an instance of the class
   *
   * @param emitter_type - the type of the emitter
   * @param emitter_pos - the position of the emitter
   * @param emitter_p1 - Custom variable 1 for the emitter. Purpose depends on the shape.
   * @param emitter_p2 - Custom variable 2 for the emitter. Purpose depends on the shape.
   */
  constructor(shape, pos, p1, p2) {
    super(shape, pos, p1, p2);
    this.shape = shape;
    this.pos = pos;
  }
  /**
   * Validate the object properties
   *
   * @returns
   */
  validate() {
    return validate(this, _EmitterConfig.rules);
  }
};

// src/index.ts
function hello_particles() {
  return "particles ready changed \u{1F386}";
}
function init(canvas, particle_effect, emitter_config) {
  return ParticleEngine.init(canvas, particle_effect, emitter_config);
}
export {
  BoidsEffect,
  EmitterConfig,
  EmitterConfigPatch,
  EmitterShape,
  FireworksEffect,
  ParticleEngine,
  ParticleType,
  ScatterFadeEffect,
  ScatterSwirlEffect,
  hello_particles,
  init
};
